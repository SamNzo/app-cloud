...
export function startMyServer(server: http.Server, port: number) {
  // Start server
  server.listen(port); // the server object listens on port;
}

export function closeMyServer(server: http.Server) {
    // Close server
    server.close();
}
